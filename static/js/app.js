...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const filesModalCloseBtn = document.getElementById('files-modal-close-btn');
  ...
  function renderFileTree(tree, parentElement) {
    ...
    function createNode(node) {
      ...
      if (node.type === 'directory' && node.children) {
        ...
        // Collapse folder on click of arrow or folder icon
        const toggleEls = rowEl.querySelectorAll('.folder-arrow, .node-icon');
      ...
      // Copy path when clicking text name or copy badge
      rowEl.addEventListener('click', (e) => {
        ...
        copyTextToClipboard(path, `Copied path to clipboard: ${path}`);
      });
    ...
    tree.forEach(node => {
      const rootNode = createNode(node);
      rootNode.classList.add('tree-node-root');
      parentElement.appendChild(rootNode);
    });
  }

  // Load File Tree JSON data from backend
  async function loadFileTree() {
    try {
      fileTreeContainer.innerHTML = '<div style="color:var(--text-secondary);text-align:center;padding:1.5rem;">Loading workspace structure...</div>';
      const response = await fetch('/api/files');
      if (!response.ok) throw new Error(`HTTP error ${response.status}`);
      const res = await response.json();
      if (res.status === 'success') {
        renderFileTree(res.data, fileTreeContainer);
      } else {
        throw new Error(res.message || "Unknown error loading files");
      }
    } catch (err) {
      console.error(err);
      fileTreeContainer.innerHTML = `<div style="color:var(--accent-red);text-align:center;padding:1.5rem;">Failed to load files: ${err.message}</div>`;
    }
  }
  ...
  const closeFiles = () => filesModal.classList.remove('visible');
  closeFilesModal.addEventListener('click', closeFiles);
  filesModalCloseBtn.addEventListener('click', closeFiles);
